...
Teensy Serial (USB):
- Connect Teensy LC to Pi USB port
- Auto-detects on /dev/ttyACM1, /dev/ttyACM2, /dev/ttyUSB0
- Protocol: START1-4, STOP1-4, SP=xxx (commands)
- Returns JSON status with state: IDLE/RUNNING/COMPLETE/FAILED

CSV Format:
arm,waist,gripper,gripper_type,j1,j2,j3,j4,j5,j6,delay,aux1,aux2,aux3,aux4,aux5

v201 Changes:
- Teensy LC serial integration via USB (/dev/ttyACM1)
- New teach commands: t1, t2, t3, t4 (record pump channel actions)
- New teach command: ts (show Teensy status)
- Replay handles Teensy actions with START/COMPLETE/TIMEOUT
- CSV format updated with 'type' and 'teensy_ch' columns
- Home position save/restore feature
...
def main():
    print("="*70)
    print("  MyBuddy 2 Training & Replay System v201_R")
